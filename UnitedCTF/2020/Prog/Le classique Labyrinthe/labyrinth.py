...
def parse(l):
	...
	for c in l:
		ret.append(Case(False if c == '#' else True))

	return ret

nc = netcat.Netcat(('challenges.unitedctf.ca', 3006))


print('Fetching labyrinth from server...')
data = [list(l) for l in nc.recv().decode().split('\n')[1:-1]]
print('Done')

print('Parsing received data into something usable...')
a = np.array(data)
data = list(map(parse, data))
start = np.argwhere(a == 'D')[0].tolist()[::-1]
finish = np.argwhere(a == 'F')[0].tolist()[::-1]
print('Done\n')
...
if path_start == f'{start[0]} {start[1]}' and path_finish == f'{finish[0]} {finish[1]}':
	path.insert(0, str(len(path)))
	path = '\n'.join(path)
	nc.send(path)
	print(nc.recv().decode())
else:
	print('Labyrinth solving failed, please try again...')
